...
verification workflows, currently focused on versioned education and veteran
verification routes plus Redis-backed health/status behavior.
...
- `api/handlers`: HTTP handlers for Redis health, spec serving, education verification, and veteran verification.
- `api/middleware`: development identity injection and circuit-breaker middleware.
...
- Interface-driven boundaries for integration points (`Service`, `HTTPTransport`, `OtelService`, `Breaker`).
...
flowchart TD
    ...
    B --> C[Request ID + Slog + Recover + CORS + OTel middleware]
    C --> D{SKIP_AUTH == true?}
    D -->|Yes| E[SkipAuthMiddleware]
    ...
    G -->|Yes: POST /api/v0/education-enrollments| K[EducationHandler parses JSON]
...
- **High confidence:** `POST /api/v0/education-enrollments` and
  `POST /api/v0/veteran-disability-ratings` are the live versioned verification
  routes in the current runtime.
