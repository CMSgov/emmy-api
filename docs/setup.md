...
- Docker and Docker Compose for containerized local workflows. The committed
  compose file currently provides the API, Redis, and observability services.
...
- The current branch does not wire Cognito-specific config or auth middleware at
  runtime; `SKIP_AUTH=true` enables the local development identity middleware in
