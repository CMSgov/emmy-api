...
| [`requestbatch-10-small1.json`](requestbatch-10-small1.json) | `POST` Batch Submission | Batch request with multiple records, submitted for asynchronous response. Submitting an asynchronous batch will provide you a synchronous response body (see [`response-10-requestbatch_body.json`](response-10-requestbatch_body.json) sample below). *Note that you must provide a unique `batchId` for each submission.* |
